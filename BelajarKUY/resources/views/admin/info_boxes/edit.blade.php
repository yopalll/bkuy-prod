...
<div class="max-w-4xl">
    <div class="bg-brand-cream-card rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-slate-100">
            ...
            <p class="text-sm text-slate-400 mt-1">Perbarui detail di bawah ini.</p>
        </div>

        <form action="{{ route('admin.info-boxes.update', $infoBox) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="description"
                          ...
                          class="w-full bg-brand-bg-soft border border-slate-200 rounded-2xl px-5 py-4 text-sm outline-none focus:ring-4 focus:ring-brand-accent-blue/20 focus:border-brand-accent-blue/40 transition-all">{{ old('description', $infoBox->description) }}</textarea>
                @error('description')
                    <p class="text-rose-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    ...
                    <input type="number"
                           name="order_position"
                           value="{{ old('order_position', $infoBox->order_position) }}"
                           min="0"
                           class="w-full bg-brand-bg-soft border border-slate-200 rounded-2xl px-5 py-4 text-sm outline-none focus:ring-4 focus:ring-brand-accent-blue/20 focus:border-brand-accent-blue/40 transition-all">
