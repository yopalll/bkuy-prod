...
<div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-2">
            Perbarui informasi banner slider.
        </p>
    </div>
...
<div class="max-w-4xl">
    <div class="bg-brand-cream-card rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-slate-100">
            ...
            <p class="text-sm text-slate-400 mt-1">Perbarui detail banner slider di bawah ini.</p>
        </div>

        <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number"
                           name="order_position"
                           value="{{ old('order_position', $slider->order_position) }}"
                           min="0"
                           class="w-full bg-brand-bg-soft border border-slate-200 rounded-2xl px-5 py-4 text-sm outline-none focus:ring-4 focus:ring-brand-accent-blue/20 focus:border-brand-accent-blue/40 transition-all">
            ...
            <div class="mb-8">
                <label class="block text-sm font-semibold text-brand-text-dark mb-3">
                    Banner Gambar <span class="text-xs text-slate-400 font-normal">(Opsional, biarkan kosong jika tidak ingin mengubah)</span>
                </label>
                ...
                <div class="w-full border-2 border-dashed border-slate-300 rounded-3xl bg-brand-bg-soft p-10 flex flex-col items-center justify-center relative hover:bg-brand-bg-soft/80 transition cursor-pointer group" onclick="document.getElementById('image').click()">
                    ...
                    <img id="image-preview" src="{{ $slider->image_url }}" alt="Preview" class="absolute inset-0 w-full h-full object-cover rounded-3xl z-10" />
                    ...
                    <div id="upload-placeholder" class="flex flex-col items-center z-20 group-hover:scale-105 transition-transform opacity-0 hover:opacity-100 bg-black/40 absolute inset-0 rounded-3xl justify-center">
                        ...
                        <h3 class="text-sm font-semibold text-white">Klik untuk mengganti gambar</h3>
                    </div>
