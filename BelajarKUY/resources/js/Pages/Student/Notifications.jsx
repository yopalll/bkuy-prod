...
function NotificationItem({ notif }) {
    ...
    return (
        <div className={`bg-surface rounded-2xl p-lg flex gap-md items-start shadow-sm hover:shadow-md transition-shadow cursor-pointer relative border border-transparent hover:border-surface-variant ${unread ? '' : 'opacity-80'}`}>
            {unread && (
                ...
                {notif.action_label && notif.action_url && (
                    <div className="mt-sm">
                        <a
                            ...
                            className="bg-secondary-container text-on-secondary-container font-label-md text-label-md px-4 py-2 rounded-lg hover:bg-secondary-fixed transition-colors inline-block"
                        >
                            {notif.action_label}
                        </a>
                    </div>
            ...
            {items.length === 0 ? (
                ...
                <div className="px-margin-mobile md:px-margin-desktop py-xl md:py-xxl max-w-4xl mx-auto">
                    ...
                    <div className="flex justify-between items-end mb-xl pb-md border-b border-surface-variant">
                        ...
                        {unreadCount > 0 && (
                            <button className="text-primary font-label-md text-label-md hover:underline flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-md">
                        {items.map((notif, i) => (
                            <NotificationItem key={notif.id ?? i} notif={notif} />
                        ))}
                    </div>
                </div>
            )}
